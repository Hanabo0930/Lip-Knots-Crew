...
export default function AdminSubmissionPreview({file,busy,onReload}:{file:FilePreview;busy:boolean;onReload:()=>Promise<void>}){
  ...
  if(file.contentType==="application/pdf")return <PdfFilePreview key={JSON.stringify([file.submissionId,file.id])} url={file.previewUrl} name={file.driveName||file.originalName||"報告書PDF"} busy={busy} onRefresh={onReload}/>;
  ...
  return <><img src={file.previewUrl} alt={file.driveName||file.originalName} loading="lazy" decoding="async" onError={()=>setFailed(true)}/><AdminImageViewer src={file.previewUrl} name={file.driveName||file.originalName}/></>;
}
