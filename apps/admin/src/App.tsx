...
<section className="panel safe-edit-panel" id="job-safe-edit">
  <div className="section-heading">
    <div><h2>案件の安全編集</h2><p>スタッフ、基本情報、請求・支払の入力セルだけを変更します。合計・数式セルはロックされています。</p></div>
    <strong>Revision {jobEditRevision}</strong>
  </div>
  <div className="job-form-grid">
    <label>対象案件<select value={jobEditId} onChange={(e)=>{const job=jobs.find((item)=>item.id===e.target.value);if(job)loadJobEdit(job);}}>{jobs.map((job)=><option key={job.id} value={job.id}>{job.workDate} {job.storeName}</option>)}</select></label>
    <label>スタッフ<select value={jobEdit.assignedStaffId} onChange={(e)=>updateJobEdit("assignedStaffId",e.target.value)}><option value="">未手配</option>{staff.filter((profile)=>profile.active!==false).map((profile)=><option key={profile.id} value={profile.id}>{profile.displayName}</option>)}</select></label>
    <label>クライアント<input value={jobEdit.clientName} onChange={(e)=>updateJobEdit("clientName",e.target.value)} /></label>
    <label>店舗<input value={jobEdit.storeName} onChange={(e)=>updateJobEdit("storeName",e.target.value)} /></label>
    <label>メーカー<input value={jobEdit.makerName} onChange={(e)=>updateJobEdit("makerName",e.target.value)} /></label>
    <label>メニュー<input value={jobEdit.menuName} onChange={(e)=>updateJobEdit("menuName",e.target.value)} /></label>
    <label>入店時間<input value={jobEdit.entryTime} onChange={(e)=>updateJobEdit("entryTime",e.target.value)} /></label>
    <label>実施時間<input value={jobEdit.workTime} onChange={(e)=>updateJobEdit("workTime",e.target.value)} /></label>
    <label>外注名<input value={jobEdit.subcontractorName} onChange={(e)=>updateJobEdit("subcontractorName",e.target.value)} /></label>
  </div>
  <h3>請求側 S～Z</h3>
  <div className="money-grid">{invoiceLabels.map(([key,label])=><label key={key}>{label}<input inputMode="numeric" value={jobEdit[key]} onChange={(e)=>updateJobEdit(key,e.target.value)} /></label>)}</div>
  <h3>支払側 AB～AI</h3>
  <div className="money-grid">{staffPayLabels.map(([key,label])=><label key={key}>{label}<input inputMode="numeric" value={jobEdit[key]} onChange={(e)=>updateJobEdit(key,e.target.value)} /></label>)}</div>
  <div className="locked-note">🔒 AA・AJ・AR・BBなどの合計／数式セルは直接編集しません。</div>
  <div className="sync-actions"><button onClick={saveJobEdit} disabled={jobEditBusy}>{jobEditBusy?"保存中…":"入力セルだけ保存"}</button></div>
</section>
