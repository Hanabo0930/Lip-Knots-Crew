...
import {parseCampaignImportText,campaignFromImport,type CampaignImportSource} from "./campaign-import";
import type {MailCampaign} from "./campaign-registration";
import {useEffect,useLayoutEffect,useRef,useState} from "react";
import {IMPORT_TARGET_LIMIT,appImportSnapshotResult,type ImportTargets,type ImportSnapshotResult} from "./import-snapshot";
...
export default function ImportSnapshotPanel({companyId,uid,api,registrationApi,demo=false,demoTargets,demoSource,onClose}:{companyId:string;uid:string;api:ImportSnapshotApi;registrationApi?:CampaignApi;demo?:boolean;demoTargets?:ImportTargets;demoSource?:unknown;onClose:()=>void}){
 ...
 const [source,setSource]=useState<CampaignImportSource|null>(null),[prepared,setPrepared]=useState<MailCampaign|null>(null);
 ...
 function invalidate(){setSource(null);setPrepared(null);version.current++;reading.current=false;setTargets(null);setResult(null);setLoading(false);setError("");setMessage("");}
 ...
 async function read(){
  ...
  try{const input=parseCampaignImportText(text,companyId),selected=input.targets,loaded=appImportSnapshotResult(await apiRef.current.read(selected),selected);
   if(!isCurrent()||ticket!==version.current)return;focusPending.current={owner,ticket};setTargets(selected);setResult(loaded);setSource(input);

  }catch(failure){if(isCurrent()&&ticket===version.current)setError(messageOf(failure));}
  finally{if(isCurrent()&&ticket===version.current){reading.current=false;setLoading(false);}}
 }
 async function continueRegistration(){
  if(!isCurrent()||loading||!source?.sourceHash||!result||!registrationApi)return;
  const ticket=++version.current;setLoading(true);setError("");
  try{const campaign=await campaignFromImport(source,result,companyId);if(isCurrent()&&ticket===version.current)setPrepared(campaign);}
  catch(failure){if(isCurrent()&&ticket===version.current)setError(messageOf(failure));}
 ...
 if(prepared&&registrationApi)return <CampaignRegistrationPanel companyId={companyId} uid={uid} api={registrationApi} demo={demo} initialCampaign={prepared} onClose={()=>{setPrepared(null);requestAnimationFrame(()=>heading.current?.focus());}}/>;
 return <section className="campaign-panel" aria-label="アプリの照合データ">
  ...
  <div className="campaign-input">
   ...
   <label>募集対象データ<textarea aria-label="募集対象データ" value={text} maxLength={IMPORT_TARGET_LIMIT} spellCheck={false} onChange={event=>{invalidate();setText(event.target.value);setFilename("");}} placeholder="連携用のimport-source.json"/></label>
   <p className="campaign-hint">案件メール側の連携ファイルを選びます。旧形式のtargets.jsonも照合データの取得に使えます。本文・宛先やログイン情報をここへ貼り付けないでください。</p>
   <div className="campaign-actions"><button disabled={loading||!text.trim()} onClick={()=>void read()}>{loading?"現在の対応を確認しています…":"現在の照合データを取得"}</button>
    {demo&&demoTargets&&<button className="ghost" disabled={loading} onClick={()=>{invalidate();setText(JSON.stringify(demoSource??demoTargets,null,2));setFilename("デモの募集対象");}}>デモの募集対象を使う</button>}</div>
  </div>
  {result&&targets&&<div className="campaign-preview">
   ...
   <p className="campaign-hint">{source?.sourceHash&&registrationApi?"取得した案件を使って、このまま募集登録の確認へ進めます。次の画面で最新の案件と原本を確認して登録します。":"取得時点の照合データです。旧形式の場合は保存して元の募集案と合わせるか、案件メール側で新しい連携ファイルを生成してください。"}</p>
   {source?.sourceHash&&registrationApi&&<button onClick={()=>void continueRegistration()} disabled={loading}>{loading?"募集内容を準備しています…":"この内容で募集登録へ進む"}</button>}
