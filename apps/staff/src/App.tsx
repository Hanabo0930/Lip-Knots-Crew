...
export default function App(){
  ...
  async function chooseSubmission(type:SubmissionType,job:Job,req=""){setSelectedJob(job);setSubmissionType(type);setRequestId(req);setResubmissionDetail(null);await loadSubmissionHistory(job.id,type);if(req)await loadResubmissionDetail(req);setView("submit");}
  ...
  return <main className="app-shell">
    ...
    {view==="home"&&<>
      ...
      <section><h2>次回シフト</h2>{activeJob?<article className="job"><span className="date">{activeJob.workDate||activeJob.dateKey}</span><h3>{activeJob.storeName}</h3><p>{activeJob.makerName} / {activeJob.menuName}</p><button onClick={()=>{setSelectedJob(activeJob);setView("shifts")}}>シフトを開く</button></article>:<div className="empty">確定シフトはありません。</div>}</section>
    </>}
    ...
    {view==="shifts"&&<section><h2>自分のシフト</h2><div className="grid">{myJobs.map(job=><article className="job" key={job.id} onClick={()=>setSelectedJob(job)}><span className="date">{job.workDate||job.dateKey}</span><h3>{job.storeName}</h3><p>{job.workTime}</p></article>)}</div>{selectedJob&&<section className="panel"><h2>{selectedJob.storeName}</h2><div className="form-grid"><label>体温<input value={temperature} onChange={e=>setTemperature(e.target.value)}/></label><label>到着予定時刻<input value={arrivalTime} onChange={e=>setArrivalTime(e.target.value)}/></label></div><button onClick={submitPreContact}>事前連絡を送信</button><hr/><h3>ネットプリント</h3>{(selectedJob.netPrint?.items??[]).map(item=><div className="netprint-row" key={item.id}><strong>{item.number}</strong><button className={item.printed?"secondary":""} disabled={item.printed} onClick={()=>markPrinted(item)}>{item.printed?"印刷済み":"印刷しました"}</button></div>)}<hr/><div className="submission-actions"><button onClick={()=>chooseSubmission("sales_floor",selectedJob)}>売場画像を提出</button><button onClick={()=>chooseSubmission("report",selectedJob)}>報告書を提出</button></div></section>}</section>}
    {view==="submit"&&<section className="panel"><h2>{submissionType==="report"?"報告書":"売場画像"}を提出</h2><p>{selectedJob?.storeName}{requestId&&" / 再提出依頼への対応"}</p>
      ...
      <div className="upload-box"><input type="file" multiple={!requestId} accept="image/*,.pdf" onChange={e=>setFiles(Array.from(e.target.files??[]).slice(0,requestId?1:20))}/><small>{requestId?"再送対象は1ファイルだけ選択してください":"売場画像・報告書それぞれ最大20件、1件50MB"}</small></div><div className="file-list">{files.map(file=><div key={`${file.name}_${file.lastModified}`}><span>{file.name}</span><em>{uploadState[file.name]??"下書き保存済み"}</em></div>)}</div><button onClick={uploadSubmission} disabled={!files.length}>{requestId?"この画像を再送する":"全ファイルを送信する"}</button>
