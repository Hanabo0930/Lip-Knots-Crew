...
export default function App(){
  ...
  async function apply(job:Job){
    ...
    try{
      await run("apply-action",async()=>{
        ...
        try{
          if(!firebaseConfigured){accepted=true;applicationResultFocusRef.current={authVersion:authLoadVersion,navigationVersion};setAcceptedApplicationJobId(job.id);setMessage("デモ：応募が確定しました。");setOpenJobs(v=>v.filter(x=>x.id!==job.id));setMyJobs(v=>[...v.filter(x=>x.id!==job.id),{...job,status:"assigned"}]);setExpandedOpenJobId("");return;}
          ...
          const attempt=await reserveApplicationAttempt(owner,job.id,()=>applicationAttemptsRef.current.get(job.id)??{requestId:crypto.randomUUID(),startedAt:Date.now(),...(job.mailApplication?{mailApplicationId:job.mailApplication.id,mailApplicationRevision:job.mailApplication.revision}:{})},isCurrentAction);
          ...
          let response;
          try{response=await httpsCallable(functions,"applyToJob")({jobId:job.id,requestId:attempt.requestId,...(attempt.mailApplicationId?{mailApplicationId:attempt.mailApplicationId,mailApplicationRevision:attempt.mailApplicationRevision}:{})});}
          catch(error){
          ...
          catch{if(isCurrentAction())setMessage("応募は確定しています。募集一覧の更新を確認できませんでした。「募集案件を更新」で最新情報を確認してください。");}
          if(isCurrentAction()&&!assignedShiftLoaded)setMessage("応募は確定しています。シフトへの反映を確認できません。「シフト」で「シフトを更新」を押して確定状況を確認してください。");
    ...
    try{
      await run("shift-action",async()=>{
        ...
        try{
          ...
          const response=await httpsCallable(functions,"submitPreContact")({jobId,dateKey,temperature:Number(temperature),arrivalTime});
          if(!isCurrentAction())return;
    ...
    try{
      await run("shift-action",async()=>{
        ...
        try{
          ...
          const response=await httpsCallable(functions,"markNetPrintPrinted")({jobId,itemId:item.id,dateKey});
          if(!isCurrentAction())return;
          const result=response.data as {ok?:unknown}|null;
    ...
    try{
      await run("shift-action",async()=>{
        ...
        try{
          ...
          const response=await httpsCallable(functions,"setSalesFloorClientSubmitted")({jobId,submitted:value});
          if(!isCurrentAction())return;
          const result=response.data as {ok?:unknown}|null;
  ...
  async function uploadSubmission(){
    ...
    try{
      await run("uploadSubmission",async()=>{
        ...
        const r=await httpsCallable(activeFunctions,"createUploadSession")({jobId,type:currentType,purpose,resubmissionRequestId:currentRequestId||undefined,files:files.map(f=>({originalName:f.name,contentType:submissionFileContentType(f),size:f.size}))});
        if(!isCurrentUpload())return;
        const data=r.data as {submissionId:string;files:{storagePath:string}[]};
        if(!data||typeof data.submissionId!=="string"||!data.submissionId.trim()||!Array.isArray(data.files)||data.files.length!==files.length||data.files.some(target=>!target||typeof target.storagePath!=="string"||!target.storagePath.trim())||new Set(data.files.map(target=>target.storagePath)).size!==files.length)throw new Error("送信先を正しく準備できませんでした。もう一度お試しください。");
        ...
        await runWithConcurrency(uploads,UPLOAD_CONCURRENCY,async({target,file})=>{
          ...
          await new Promise<void>((resolve,reject)=>uploadBytesResumable(ref(activeStorage,target.storagePath),file,{contentType:submissionFileContentType(file)}).on(
            "state_changed",
          ...
          throw new Error("送信を完了できませんでした。選択ファイルは保持しています。「提出情報を再読み込み」で履歴を確認してから、未提出分を選び直してください。");
        }
        if(!isCurrentUpload())return;
  ...
  return <main className="app-shell">
    ...
    {view==="submit"&&selectedAssignedJob&&<section ref={submissionPanelRef} tabIndex={-1} aria-label="提出画面" className={`panel submission-panel ${submissionType}`} aria-busy={submissionEditPending}>
      ...
      {resubmissionDetail&&<div className="resubmission-guide"><div><strong>再送理由</strong><p>{resubmissionDetail.request.reasons.map(reason=>reason.trim()).filter(Boolean).join(" / ")||"再送理由を確認できません。「提出情報を再読み込み」で確認し、改善しない場合は管理者に撮り直す内容を確認してください。"}</p>{resubmissionDetail.request.note.trim()&&<p>{resubmissionDetail.request.note}</p>}</div><div className="source-preview"><span>{resubmissionDetail.request.status==="open"?"撮り直す元画像":"依頼の対象画像"}</span>{resubmissionDetail.source?<SubmissionPreviewImage file={resubmissionDetail.source} onRefreshPreview={refreshFilePreview} className="source-preview-frame"/>:<div className="preview-placeholder" role="status">元画像を確認できません。「提出情報を再読み込み」で確認し、改善しない場合は管理者に対象ファイルを確認してください。</div>}</div><small>{resubmissionDetail.request.status==="submitted"?"この依頼への再送は受付済みです。提出履歴で保存状況を確認してください。":resubmissionDetail.request.status==="completed"?"この再提出依頼は完了しています。提出履歴で内容を確認できます。":resubmissionDetail.request.status!=="open"?"再提出依頼の受付状態を確認できません。「提出情報を再読み込み」で確認してください。":resubmissionDetail.source?"この画像だけを撮り直し、1ファイル選んで再送してください。":"再送する対象を確認してから、1ファイル選んで再送してください。"}</small></div>}
      {submissionType==="sales_floor"&&<><p id="client-submission-help" className="muted">クライアントへ直接提出した場合は、ここで提出済みを記録してください。</p><button className="secondary" onClick={()=>{if(!submissionEditPending)void setClientSubmitted(selectedAssignedJob.submissionStatus?.salesFloor?.clientSubmitted!==true);}} aria-describedby="client-submission-help" aria-disabled={submissionEditPending} aria-busy={shiftActionPending}>{pendingShiftAction==="clientSubmitted"?"更新中…":selectedAssignedJob.submissionStatus?.salesFloor?.clientSubmitted===true?"クライアント提出を解除":"クライアントへ提出済み"}</button></>}
      <div className="upload-box"><span className="submission-step-label">3. 写真・PDFを選ぶ</span><div className="file-picker-actions"><label className="file-picker-button camera">📷 カメラで撮影<input className="file-picker-input" type="file" accept="image/*" capture="environment" disabled={submissionEditPending||resubmissionSendBlocked} onChange={e=>{addSubmissionFiles(Array.from(e.target.files??[]));e.currentTarget.value="";}}/></label><label className="file-picker-button library">🖼️ 写真・PDFを選ぶ<input className="file-picker-input" type="file" multiple={!requestId} accept="image/*,.pdf" disabled={submissionEditPending||resubmissionSendBlocked} onChange={e=>{addSubmissionFiles(Array.from(e.target.files??[]));e.currentTarget.value="";}}/></label></div><small>{requestId?"再送対象は1ファイルだけ選択してください":`${submissionType==="report"?"報告書":"売場画像"}として最大20件、1件50MB。選択後も追加できます。`}</small></div>
