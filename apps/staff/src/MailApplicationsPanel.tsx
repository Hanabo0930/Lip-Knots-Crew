...
type Job={revision?:number;menuConditions?:string[];id:string;workDate:string;dateKey:string;clientName:string;makerName:string;menuName:string;
...
export function parseMailApplications(value:unknown):Page{
  ...
  const items=data.items.map((raw:unknown)=>{
    ...
    if(job.revision!==undefined&&(!Number.isSafeInteger(job.revision)||job.revision<0))throw invalid();
    if(job.menuConditions!==undefined&&(!Array.isArray(job.menuConditions)||job.menuConditions.some(value=>typeof value!=="string")))throw invalid();
...
export default function MailApplicationsPanel({load,onApply,onViewShift,onClose,busy,hasAttempt,feedback}:{
  ...
  return <section className="mail-applications" aria-busy={disabled||status==="loading"} aria-label="メールからの応募">
    ...
    <p>メールで申し込んだ案件を確認できます。内容を確認して応募してください。受付後は、シフトで担当の確認状況を確認できます。</p>
    ...
    {lastAttempt&&<div className="empty" role="status"><p>{accepted?"応募を受け付けました。シフトで担当の確認状況を確認してください。":pending?"応募結果を確認しています。":attemptFeedback||feedback||"応募結果を確認できません。再確認するか、シフトで現在の状況を確認してください。"}</p>
    ...
    <div className="grid">{page.items.map(item=>{
      ...
      return <article className="job" key={item.id} aria-label={item.job.dateKey+" "+(item.job.storeName||"店舗確認中")+"のメール応募"}>
        ...
        {Boolean(item.job.menuConditions?.length)&&<p>勤務条件：{item.job.menuConditions!.join(" / ")}</p>}
        ...
        {item.state==="needs_review"?<p role="status">案件または本人情報が変更されています。管理者へ確認してください。</p>:
          item.state==="assigned"||confirmed?<><p role="status">アプリで応募を受付済みです。シフトで担当の確認状況を確認してください。</p><button className="secondary" disabled={disabled} onClick={()=>onViewShift(item.job.id)}>シフトを確認</button></>:
          <><p>日付・店舗・勤務条件を確認してから応募してください。</p><button disabled={disabled||status!=="ready"} onClick={()=>void apply(item)}>
            {pending===item.id?"応募を確認中…":hasAttempt(item.job.id)?"前回の応募結果を再確認":"この内容で応募する"}</button></>}
      </article>;
    })}</div>
