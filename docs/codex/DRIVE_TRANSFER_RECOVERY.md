...
## 2026-09-16: 提出開始と転送側の同版反映

createUploadSessionをSTAGING限定配備へ追加し、提出開始だけを反映する場合も、実稼働finalizeStagedUploadの転送状態をpreserveで継承する。入口の停止分岐を持たないソースはpaused/acceptanceで配備できない。通常の停止/再開/専用受入切替はcreateUploadSession,finalizeStagedUploadの2関数を指定して同じ設定へそろえる。他関数を混在した明示切替や、入口だけの明示切替は拒否する。従来のfinalizeStagedUpload単独操作は保持するが、入口の状態まで変えたと判断しない。

実配備の両状態・最新revisionへの切替を確認するまでは、受付と転送がそろったと報告しない。停止解除で既存イベントが再送されるわけではなく、受信済みの通常提出の再処理は別の照合・承認範囲とする。
