...
export const createResubmissionRequest = onCall(async request => {
 ...
 const jobRef=db.collection("jobs").doc(input.jobId),job=await jobRef.get();
 if(!job.exists||job.data()?.companyId!==companyId)throw new HttpsError("not-found","案件が見つかりません。");
 const staffId=String(job.data()?.assignedStaffId??"");
 if(!staffId)throw new HttpsError("failed-precondition","担当スタッフがいません。");
 const ref=db.collection("resubmissionRequests").doc();
 const notification = {companyId,targetStaffId:staffId,title:input.type==="report"?"報告書を再送してください":"売場画像を再送してください",body:[...input.reasons,input.note].filter(Boolean).join(" / "),route:`/resubmissions/${ref.id}`,category:"resubmission_request",dedupeKey:ref.id};
 const notificationRef=db.collection("notificationQueue").doc(notificationQueueId(notification));
 await db.runTransaction(async tx=>{
  const current=await tx.get(jobRef);
  if(!current.exists||current.data()?.companyId!==companyId)throw new HttpsError("not-found","案件が見つかりません。");
