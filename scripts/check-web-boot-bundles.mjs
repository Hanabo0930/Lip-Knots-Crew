...
import {runPwaUpdateModule} from "./pwa-update-test-harness.mjs";
...
for (const app of apps) {
  ...
  await verifyPwaUpdateBehavior(app, pwaUpdateSource);
...
function verifyPwaUpdateLifecycle(app, source) {
 const checks=[
  ['intercepts library-triggered automatic reload',source.includes('onNeedReload:()=>offerUpdate(true)')],
  ['does not reload or activate from update detection',!source.includes('window.location.reload')&&!source.includes('postMessage(')],
  ['offers an already waiting update',source.includes('current.waiting&&navigator.serviceWorker.controller')],
  ['checks for updates on foreground return',source.includes('document.addEventListener("visibilitychange",checkForUpdates)')&&source.includes('current.update()')],
 ];
 for(const[label,passed]of checks)if(!passed)failures.push(app+': PWA lifecycle '+label);
}
async function verifyPwaUpdateBehavior(app,source){
 const result=await runPwaUpdateModule(source),first=await runPwaUpdateModule(source,null,Date.now(),'',false);
 const checks=[
  ['defers activation and reload until an explicit action',result.waitingMessages===0&&result.reloads===0],
  ['coalesces duplicate startup and foreground checks',result.registered===1&&result.updateChecks===1],
  ['intercepts workbox reload and offers a ready update',result.libraryReloadIntercepted&&result.offers.some(offer=>offer.active)],
  ['preserves first-installation input without update notice',first.initialOffers===0&&first.firstInstallOffers===0&&first.reloads===0],
 ];
 for(const[label,passed]of checks)if(!passed)failures.push(app+': executable PWA update '+label);
}