...
if (!['business','rules','mail','login-cleanup','row-queue','row-success','creation-journey','received-staff-journey','admin-creation-audit','admin-mutation-audit','native-creation-retry','mail-cancellation-journey'].includes(suite)) throw new Error('UNKNOWN_LOCAL_ACCEPTANCE_SUITE');
...
try {
  ...
  const child=spawn(process.execPath,[path.join(root,suite === 'rules' ? 'scripts/test-local-firestore-rules.mjs' : suite === 'mail' ? 'scripts/test-local-case-mail-acceptance.mjs' : suite === 'login-cleanup' ? 'scripts/test-local-login-cleanup.mjs' : suite === 'row-queue' ? 'scripts/test-local-row-queue.mjs' : suite === 'row-success' ? 'scripts/test-local-row-success.mjs' : suite === 'mail-cancellation-journey' ? 'scripts/test-local-mail-cancellation-journey.mjs' : suite === 'native-creation-retry' ? 'scripts/test-local-native-creation-retry.mjs' : suite === 'admin-mutation-audit' ? 'scripts/test-local-admin-mutation-audit.mjs' : suite === 'admin-creation-audit' ? 'scripts/test-local-admin-creation-audit.mjs' : suite === 'received-staff-journey' ? 'scripts/test-local-received-staff-journey.mjs' : suite === 'creation-journey' ? 'scripts/test-local-creation-journey.mjs' : 'scripts/test-local-firestore-acceptance.mjs'),path.join(evidence,'result.json')],{
