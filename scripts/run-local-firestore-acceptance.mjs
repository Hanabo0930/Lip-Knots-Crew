...
if (!['business','rules','mail','login-cleanup','row-queue','row-success','creation-journey'].includes(suite)) throw new Error('UNKNOWN_LOCAL_ACCEPTANCE_SUITE');
...
try {
  ...
  const child=spawn(process.execPath,[path.join(root,suite === 'rules' ? 'scripts/test-local-firestore-rules.mjs' : suite === 'mail' ? 'scripts/test-local-case-mail-acceptance.mjs' : suite === 'login-cleanup' ? 'scripts/test-local-login-cleanup.mjs' : suite === 'row-queue' ? 'scripts/test-local-row-queue.mjs' : suite === 'row-success' ? 'scripts/test-local-row-success.mjs' : suite === 'creation-journey' ? 'scripts/test-local-creation-journey.mjs' : 'scripts/test-local-firestore-acceptance.mjs'),path.join(evidence,'result.json')],{
