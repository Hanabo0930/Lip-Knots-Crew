...
for(const [api,cap] of [['getOperationsDashboard',15000],['getStaffPerformance',10000]])for(const count of [0,cap-1,cap,cap+1]){
 ...
 const load=name=>{if(boundary[name])return boundary[name];assert.ok(['./analytics','./analytics-core','./sheet-write-core','./assignment-preparation-core','./netprint-state-core'].includes(name),'Unexpected local dependency: '+name);if(modules.has(name))return modules.get(name);const exports={};modules.set(name,exports);runInNewContext(ts.transpileModule(fs.readFileSync('functions/src/'+name.slice(2)+'.ts','utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,{exports,require:load});return exports;};
