...
const source = fs.readFileSync(new URL('../functions/src/jobs.ts', import.meta.url), 'utf8');
...
function setup(lock, jobPatch = {}) {
  ...
  const cancel = (role = 'admin') => exports.adminCancelJob({ auth: { uid: 'synthetic-admin', token: { companyId, role } }, data: { jobId: 'old-job', reason: '合成取消' } });
...
for (const [patch, role] of [[{ companyId: 'other' }, 'admin'], [{}, 'staff']]) {
  ...
  await assert.rejects(test.cancel(role), { code: 'permission-denied' });
