...
function harness() {
  ...
  const collection = (name, filters=[]) => ({add:async data=>{write(ref(`${name}/new`),data);return ref(`${name}/new`);},doc:(id='new')=>ref(`${name}/${id}`),where:(key,op,value)=>{assert.ok(['==','in'].includes(op));return collection(name,[...filters,[key,value,op]]);},orderBy:()=>collection(name,filters),limit:()=>collection(name,filters),get:async()=>{ const docs=[...rows].filter(([p,data])=>p.startsWith(name+'/')&&filters.every(([k,v,op])=>op==='in'?v.includes(data[k]):data[k]===v)).map(([p])=>snapshot(ref(p))); await h.afterQuery?.(); return {docs,size:docs.length,empty:docs.length===0}; }});
  const db = {collection,getAll:async(...refs)=>refs.map(snapshot),runTransaction:async fn=>{await h.beforeTransaction?.();const writes=[];const value=await fn({get:async r=>{assert.equal(writes.length,0);return snapshot(r);},set:(r,d,o)=>writes.push([r,d,o]),update:(r,d)=>writes.push([r,d,{update:true}])}); await h.beforeCommit?.();for(const [r,d,o] of writes)write(r,d,o);return value;}};
  const boundary={'node:crypto':dependency('node:crypto'),'./firebase':{db},'firebase-admin/firestore':{Timestamp,FieldValue:{increment:n=>n,delete:()=>null,serverTimestamp:()=>Timestamp.fromMillis(1000)}},'firebase-functions/v2/https':{HttpsError,onCall:fn=>fn},'firebase-functions/v2/firestore':{onDocumentWritten:(_p,fn)=>fn},zod:dependency('zod'),'./utils':{requireAdmin:request=>request.auth,companyFromClaims:token=>token.companyId,requestId:()=> 'synthetic'},'./system-safety':{assertProductionOperational:async()=>{}}};
  function load(name){if(boundary[name])return boundary[name];assert.ok(['./case-mail-preparation-core','./admin-operations-core','./assignment-preparation-core','./netprint-state-core','./sheet-write-core','./shift-parser','./case-id'].includes(name),'External import refused: '+name);if(modules.has(name))return modules.get(name);return compile(name);}
  ...
  const core=load('./admin-operations-core'),values=core.normalizeExpenseInput({transportation:20}).values,before=core.normalizeExpenseInput({transportation:10}).values;
  Object.assign(rows.get('expenseReviews/job'),{values,expectedValues:before,writeContext:core.expenseSheetWriteContext(rows.get('jobs/job'))});
  Object.assign(rows.get('sheetSyncQueue/queue'),{updates:core.buildExpenseSheetUpdates(values),expected:core.buildExpenseExpected(before),idempotencyKey:'expense.review:job:1'});
...
await test('unconditional expense overwrite is refused before queue creation',async()=>{const h=harness();const input=request();input.data.confirmExistingValues=true;const before=JSON.stringify([...h.rows]);await assert.rejects(h.api.completeExpenseReview(input),{code:'failed-precondition'});assert.equal(JSON.stringify([...h.rows]),before);});
...
for(const action of ['retrySheetWriteIssue','acknowledgeSheetWriteIssue']) {
 const input=()=>({auth:{uid:'admin',token:{companyId:'company'}},data:{queueId:'queue',note:'synthetic'}});
 for(const status of ['blocked','dead_letter','retry_wait'])await test(action+' accepts current '+status,async()=>{const h=harness();h.rows.get('sheetSyncQueue/queue').status=status;await h.api[action](input());assert.equal(h.rows.get('sheetSyncQueue/queue').status,action==='retrySheetWriteIssue'?'pending':'acknowledged');assert.equal(h.rows.get('auditLogs/new').queueId,'queue');});
 for(const [name,change] of [['deleted',h=>h.rows.delete('sheetSyncQueue/queue')],['foreign company',h=>h.rows.get('sheetSyncQueue/queue').companyId='other'],['already completed',h=>h.rows.get('sheetSyncQueue/queue').status='completed']])await test(action+' rejects current '+name,async()=>{const h=harness();h.rows.get('sheetSyncQueue/queue').status='blocked';let before;h.beforeTransaction=async()=>{change(h);before=JSON.stringify([...h.rows]);};await assert.rejects(h.api[action](input()));assert.equal(JSON.stringify([...h.rows]),before);});
 for(const status of ['pending','processing','acknowledged'])await test(action+' rejects '+status,async()=>{const h=harness();h.rows.get('sheetSyncQueue/queue').status=status;const before=JSON.stringify([...h.rows]);await assert.rejects(h.api[action](input()));assert.equal(JSON.stringify([...h.rows]),before);});
 await test(action+' transaction failure leaves queue and audit unchanged',async()=>{const h=harness();h.rows.get('sheetSyncQueue/queue').status='blocked';const before=JSON.stringify([...h.rows]);h.beforeCommit=async()=>{throw new Error('synthetic transaction failure');};await assert.rejects(h.api[action](input()));assert.equal(JSON.stringify([...h.rows]),before);});
}
await test('retry rejects conflict without writes',async()=>{const h=harness();Object.assign(h.rows.get('sheetSyncQueue/queue'),{status:'blocked',errorType:'conflict'});const before=JSON.stringify([...h.rows]);await assert.rejects(h.api.retrySheetWriteIssue({auth:{uid:'admin',token:{companyId:'company'}},data:{queueId:'queue'}}));assert.equal(JSON.stringify([...h.rows]),before);});
await test('acknowledgement accepts current conflict',async()=>{const h=harness();Object.assign(h.rows.get('sheetSyncQueue/queue'),{status:'blocked',errorType:'conflict'});await h.api.acknowledgeSheetWriteIssue({auth:{uid:'admin',token:{companyId:'company'}},data:{queueId:'queue'}});assert.equal(h.rows.get('sheetSyncQueue/queue').status,'acknowledged');});

const confirmation=()=>({auth:{uid:'admin',token:{companyId:'company'}},data:{jobId:'job',expectedRevision:0}});
for(const [name,change] of [['deletion',h=>h.rows.delete('jobs/job')],['company change',h=>h.rows.get('jobs/job').companyId='other'],['status change',h=>h.rows.get('jobs/job').status='cancelled'],['staff change',h=>h.rows.get('jobs/job').assignedStaffId='other']])await test('application confirmation rejects '+name+' after read',async()=>{const h=harness();h.rows.get('jobs/job').status='assigned';let before;h.afterRead=async p=>{if(p==='jobs/job'){h.afterRead=null;change(h);before=JSON.stringify([...h.rows]);}};await assert.rejects(h.api.confirmApplication(confirmation()));assert.equal(JSON.stringify([...h.rows]),before);});
await test('application confirmation normal records audit',async()=>{const h=harness();h.rows.get('jobs/job').status='assigned';await h.api.confirmApplication(confirmation());assert.equal(h.rows.get('jobs/job').applicationAdminConfirmed,true);assert.equal(h.rows.get('auditLogs/new').action,'application.confirm');});
await test('application confirmation repeated call preserves first actor and time',async()=>{const h=harness();Object.assign(h.rows.get('jobs/job'),{status:'assigned',applicationAdminConfirmed:true,applicationAdminConfirmedBy:'first',applicationAdminConfirmedAt:Timestamp.fromMillis(1)});const before=JSON.stringify([...h.rows]);await h.api.confirmApplication(confirmation());assert.equal(JSON.stringify([...h.rows]),before);});
await test('application confirmation transaction failure leaves no partial write',async()=>{const h=harness();h.rows.get('jobs/job').status='assigned';const before=JSON.stringify([...h.rows]);h.beforeCommit=async()=>{throw Error('synthetic');};await assert.rejects(h.api.confirmApplication(confirmation()));assert.equal(JSON.stringify([...h.rows]),before);});

for(const [key,value] of [['dateKey','2026-09-22'],['caseId','changed'],['workTime','11:00-19:00'],['storeName','changed'],['cancelled',true],['sourceMissing',true],['assignmentUnresolved',true],['revision',1]])await test('application confirmation rejects changed '+key,async()=>{const h=harness();h.rows.get('jobs/job').status='assigned';let before;h.afterRead=async p=>{if(p==='jobs/job'){h.afterRead=null;h.rows.get(p)[key]=value;before=JSON.stringify([...h.rows]);}};await assert.rejects(h.api.confirmApplication(confirmation()),{code:'failed-precondition'});assert.equal(JSON.stringify([...h.rows]),before);});
for(const revision of [undefined,-1,0.5,'0',1,Number.MAX_SAFE_INTEGER+1])await test('application confirmation refuses absent malformed or stale displayed revision '+revision,async()=>{const h=harness();h.rows.get('jobs/job').status='assigned';const input=confirmation();input.data.expectedRevision=revision;const before=JSON.stringify([...h.rows]);await assert.rejects(h.api.confirmApplication(input));assert.equal(JSON.stringify([...h.rows]),before);});
for(const [key,value] of [['cancelled',true],['sourceMissing',true],['assignmentUnresolved',true],['assignedStaffId',null]])await test('application confirmation refuses invalid current '+key,async()=>{const h=harness();Object.assign(h.rows.get('jobs/job'),{status:'assigned',[key]:value});const before=JSON.stringify([...h.rows]);await assert.rejects(h.api.confirmApplication(confirmation()),{code:'failed-precondition'});assert.equal(JSON.stringify([...h.rows]),before);});
await test('admin confirmation records displayed revision without clearing source confirmation',async()=>{const h=harness();Object.assign(h.rows.get('jobs/job'),{status:'assigned',dateKey:'2026-09-20',revision:7,applicationUnconfirmed:true});const input=confirmation();input.data.expectedRevision=7;await h.api.confirmApplication(input);const j=h.rows.get('jobs/job'),a=h.rows.get('auditLogs/new');assert.equal(j.applicationAdminConfirmedRevision,7);assert.equal(j.applicationUnconfirmed,true);assert.equal(a.revision,7);assert.equal(a.assignedStaffId,'staff');assert.equal(a.dateKey,'2026-09-20');});

for(const company of ['company','other',null])await test('issue job metadata company '+company,async()=>{const h=harness();h.rows.get('sheetSyncQueue/queue').status='blocked';Object.assign(h.rows.get('jobs/job'),{companyId:company,storeName:'synthetic store'});const result=await h.api.getSheetWriteIssues({auth:{uid:'admin',token:{companyId:'company'}},data:{}});if(company==='company')assert.equal(result.issues[0].job.storeName,'synthetic store');else assert.equal(result.issues[0].job,null);});
await test('issue missing job remains visible without metadata',async()=>{const h=harness();h.rows.get('sheetSyncQueue/queue').status='blocked';h.rows.delete('jobs/job');const result=await h.api.getSheetWriteIssues({auth:{uid:'admin',token:{companyId:'company'}},data:{}});assert.equal(result.count,1);assert.equal(result.issues[0].job,null);});
