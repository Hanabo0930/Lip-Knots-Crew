...
function harness() {
  ...
  const collection = (name, filters=[]) => ({add:async data=>{write(ref(`${name}/new`),data);return ref(`${name}/new`);},doc:(id='new')=>ref(`${name}/${id}`),where:(key,op,value)=>{assert.equal(op,'==');return collection(name,[...filters,[key,value]]);},limit:()=>collection(name,filters),get:async()=>{ const docs=[...rows].filter(([p,data])=>p.startsWith(name+'/')&&filters.every(([k,v])=>data[k]===v)).map(([p])=>snapshot(ref(p))); await h.afterQuery?.(); return {docs,empty:docs.length===0}; }});
  const db = {collection,runTransaction:async fn=>{await h.beforeTransaction?.();const writes=[];const value=await fn({get:async r=>{assert.equal(writes.length,0);return snapshot(r);},set:(r,d,o)=>writes.push([r,d,o]),update:(r,d)=>writes.push([r,d,{update:true}])}); await h.beforeCommit?.();for(const [r,d,o] of writes)write(r,d,o);return value;}};
