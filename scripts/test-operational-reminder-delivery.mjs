...
await test('paused delivery preserves reminder documents and does not inspect jobs or consume queues',async()=>{const f=fixture('report',{APP_ENVIRONMENT:'staging',LKC_NOTIFICATION_DELIVERY_MODE:'paused'});await f.schedule();const before=JSON.stringify([...f.h.state.records]);f.h.state.onRead=()=>{throw Error('should not read while paused');};await f.h.trigger(f.queues()[0].id);await f.h.tick();assert.equal(JSON.stringify([...f.h.state.records]),before);assert.equal(f.h.state.sent.length,0);});
