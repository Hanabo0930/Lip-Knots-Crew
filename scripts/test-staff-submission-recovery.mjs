...
for(const scenario of ['success','cleanup-failure','receipt-unavailable','auth-changed']){
 ...
 attempt:{clientRequestId:'synthetic-attempt'},clearSubmissionAttempt:async()=>events.push('clear-attempt'),
 ...
 await state.finish();
 if(scenario==='auth-changed'){assert.equal(state.skipNextDraftSaveRef.current,false);assert.equal(events.some(event=>Array.isArray(event)&&['files','confirmed','cleanup','poll'].includes(event[0])),false);continue;}
...
function selectFiles(selected,{scope='file',requestId='request',files=[original],locked=false,globalMessage='existing',resubmissionSendBlocked=false}={}){
 const state={files,confirmed:true,uploadState:{original:'ready'},message:'existing',globalMessage,writes:0};
 const context={resubmissionDetail:{request:{scope}},resubmissionSendBlocked,submissionMessage:state.message,setMessage:fn=>{state.globalMessage=fn(state.globalMessage);},files,requestId,MAX_SUBMISSION_FILES:20,MAX_SUBMISSION_FILE_SIZE:50*1024*1024,
...

{
 ...
 for(const failed of ['history','job','tasks','detail','none','stale','unconfirmed']){
  ...
  const ctx={...submissionAttemptBoundary,functions:{},authLoadVersionRef:{current:1},submissionProcessingVersionRef:{current:0},setProcessingSubmission:v=>effects.push(v),httpsCallable:()=>async()=>({data:{status:'completed',completedFiles:1,totalFiles:1,errorMessage:null}}),showSubmissionMessage:m=>messages.push(m),loadSubmissionHistory:read('history'),refreshSelectedJob:read('job'),loadTasks:read('tasks'),loadResubmissionDetail:read('detail'),Date:{now:()=>0}};
...

{
 ...
 for(const errorMessage of [null,'','  ','転送先を確認してください']){
  const messages=[],effects=[];const ctx={...submissionAttemptBoundary,functions:{},authLoadVersionRef:{current:1},submissionProcessingVersionRef:{current:0},setProcessingSubmission:v=>effects.push(v),httpsCallable:()=>async()=>({data:{status:'error',completedFiles:0,totalFiles:1,errorMessage}}),showSubmissionMessage:m=>messages.push(m),Date:{now:()=>0}};
 ...
 const messages=[];const ctx={...submissionAttemptBoundary,functions:null,showSubmissionMessage:m=>messages.push(m)};runInNewContext(code,ctx);await ctx.pollSubmissionProcessing('job','submission','report','');assert.match(messages[0],/接続準備を確認できません/);assert.match(messages[0],/「提出情報を再読み込み」/);
...

{
 const start=app.indexOf('  async function pollSubmissionProcessing('),end=app.indexOf('  async function uploadSubmission()',start);const code=ts.transpileModule(app.slice(start,end),{compilerOptions:{target:ts.ScriptTarget.ES2022}}).outputText;
 for(const stale of [false,true]){
  const messages=[],effects=[];let finish,reads=0;const ctx={...submissionAttemptBoundary,functions:{},authLoadVersionRef:{current:1},submissionProcessingVersionRef:{current:0},setProcessingSubmission:v=>effects.push(v),httpsCallable:()=>()=>{reads++;return new Promise(resolve=>finish=resolve);},showSubmissionMessage:m=>messages.push(m),Date:{now:()=>0},sleep:()=>assert.fail('Paused transfer must release controls without polling further')};
...
for(const scope of ['file','submission',undefined]){
 const many=Array.from({length:21},(_,i)=>photo('scoped-'+i+'.pdf',50*1024*1024,'application/pdf'));
 const chosen=selectFiles(many,{scope,files:[]});assert.equal(chosen.files.length,scope==='submission'?20:1);assert.match(chosen.message,new RegExp('最大'+(scope==='submission'?20:1)));
 const appended=selectFiles([replacement],{scope});assert.equal(appended.files.length,scope==='submission'?2:1);
 const invalid=selectFiles([photo('too-large.pdf',50*1024*1024+1,'application/pdf')],{scope});assert.equal(invalid.files[0],original);assert.equal(invalid.confirmed,true);
}
console.log('Resubmission scope: file remains one; explicitly whole-submission supports twenty 50MiB file descriptors and append without lowering the limit.');
